...
echo "\",\"ChunkSequenceNumber\": 1,\"TotalApplicationSize\":" >> ${uploadJson}
...
echo "{\"TransactionId\": \"" > ${beginInstallJson}
...
echo "\",\"DeviceType\": \"2\",\"ApplicationName\": \"$4\",\"SupportedModels\": {\"Model\": [" >> ${beginInstallJson}

# Model 1 is iPhone, 3 is iPod Touch
if [ "$8" = "1" ]; then
echo "{\"ApplicationId\": $6,\"ModelId\": 1},{\"ApplicationId\": $6,\"ModelId\": 3}" >> ${beginInstallJson}
fi

# Model 2 is iPad
if [ "$9" = "1" ]; then
echo ",{\"ApplicationId\": $6,\"ModelId\": 2}" >> ${beginInstallJson}
fi

# Set push mode (auto)
echo "]},\"PushMode\": \"Auto\",\"AutoUpdateVersion\": true,\"FileName\": \"$5\",\"AppVersion\": \"" >> ${beginInstallJson}

# Add the app version
echo $7 | awk '{printf substr($0,3,4); printf "."; printf substr($0,7,7)}' >> ${beginInstallJson}

echo \"\} >> ${beginInstallJson}
